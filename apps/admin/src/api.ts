...
export function createImageUploadUrl(input: {
  purpose: ImagePurpose;
  restaurantId?: string;
  contentType: ImageContentType;
  size: number;
}): Promise<ImageUploadTicket> {
  return request("/api/v1/uploads/image-upload-url", { method: "POST", body: input });
}

export function completeImageUpload(input: {
  uploadId: string;
  purpose: ImagePurpose;
  restaurantId?: string;
  contentType: ImageContentType;
}): Promise<{ imageUrl: string; blobName: string }> {
  return request("/api/v1/uploads/complete", { method: "POST", body: input });
}

export function deleteUploadedImage(input: {
  purpose: ImagePurpose;
  restaurantId?: string;
  imageUrl: string;
}): Promise<{ deleted: boolean }> {
  return request("/api/v1/uploads/image", { method: "DELETE", body: input });
}
