...
test("an ordinary notification keeps the default sound and no channel override", () => {
  assert.deepEqual(pushPresentation("ORDER_STATUS_CHANGED", "android"), { sound: "default" });
  assert.deepEqual(pushPresentation("DELIVERY_ASSIGNED", "ios"), { sound: "default" });
});
