...
## 2026-09-09: Eight launch blockers closed

Eight items from the standing launch checklist, most already documented as
known gaps in `TRIAL_DEPLOY_CHECK.md` and prior audit passes.

### Completed

- **Seed data.** `prisma/seed.ts`'s six JOVO MARKET products now carry a
  `costPriceMinor`, closing the gap the cost-price fix (05d495e) opened: a
  freshly seeded database was unsellable because `calculateOrderQuote` fails
  closed on a missing cost price. `TRIAL_DEPLOY_CHECK.md` was left as-is — its
  claims were already accurate once the seed carries real costs again.
- **First-admin bootstrap script.** `apps/api/prisma/bootstrap-admin.ts`
  (`npm run bootstrap:admin`) creates the first `SUPER_ADMIN`, hashing with
  the exact `hashPassword` the app itself uses (imported, not reimplemented),
  and refuses outright if any `ADMIN` account already exists. Takes
  `--phone`/`--password`/`--name` or prompts (password input hidden on a TTY).
- **Admin console in production compose.** New `apps/admin/Dockerfile`
  (Vite build → nginx) and `deploy/nginx/admin.conf` (same TLS/HSTS/CSP
  posture as the customer web nginx, plus its own `/api/`+`/socket.io/` proxy
  to the `api` service), wired into `docker-compose.production.yml` as a new
  `admin` service on its own host ports (`ADMIN_HTTP_PORT`/`ADMIN_HTTPS_PORT`,
  default 8080/8443) and origin (`ADMIN_APP_URL`, added to `CORS_ORIGIN`).
  Building the image on Linux surfaced a real, pre-existing problem: the
  committed `package-lock.json` (generated on Windows) never recorded
  Rollup's Linux native binary — a known npm bug
  (npm/cli#4828) — so `vite build` failed with "Cannot find module
  @rollup/rollup-linux-x64-gnu" inside the container. Fixed by installing
  that one optional package by its exact locked version as a repair step
  in the Dockerfile, rather than deleting/regenerating the lockfile (which
  turned out to also surface an unrelated pre-existing peer-dependency
  conflict in the mobile workspace).
- **Push notifications.** New `PushSenderService` (`apps/api/src/realtime/`)
  sends through Expo's push API (`https://exp.host/--/api/v2/push/send`),
  batched at 100 messages per request, deactivating a `PushToken` whose
  ticket comes back `DeviceNotRegistered` and swallowing every other failure
  (never throws). Wired into `RealtimeGateway.sendPush`, which
  `createNotification`/`createBusinessNotification` now call alongside the
  existing socket emit — both go through `DeferredEmitter` so a push (like a
  socket event) only fires after its transaction actually commits. Every
  existing notification call site picked this up for free with no changes of
  its own, since they all already go through `createNotification`.
- **Rate limit.** Default `RATE_LIMIT_LIMIT` raised 60 → 400 (both the
  `environment.ts` default and the `ThrottlerModule` fallback, which must
  agree), and `.env.production.example` updated with the same value and a
  comment explaining why (CGNAT sharing one public IP across many customers
  on Palestinian mobile carriers). Still fully configurable via env.
- **EAS Update.** `app.json`: `updates.enabled: true`, `runtimeVersion`
  policy `appVersion`, and the real `u.expo.dev` URL. `eas.json`'s `preview`
  profile no longer hardcodes the dead ngrok tunnel URL — it now relies
  entirely on `"environment": "preview"` (EAS-hosted env vars), so a build
  with no `EXPO_PUBLIC_API_URL` configured fails loudly at build time instead
  of shipping an app that can never reach an API. Since `apps/mobile/android`
  is a hand-maintained native project (not regenerated by `expo prebuild`),
  the three `expo.modules.updates.*` meta-data entries in
  `AndroidManifest.xml` were updated by hand to match, including the
  `expo-channel-name: production` request-header override this checked-in
  manifest needs (documented inline: a preview/development build from this
  same native project would need that value changed first). README gained a
  short "Mobile OTA updates" section with the `eas env:create`/`eas update`
  commands.
- **Web map CSP.** Both `location-map.web.tsx` (mobile) and `LeafletPicker.tsx`
  (admin) loaded Leaflet from `unpkg.com` — mobile's inside a `srcDoc` iframe
  whose CSP is inherited from the parent, so a production `script-src 'self'`
  blocked *both* the remote `<script>` tag and the inline map script that ran
  inside it, leaving a blank box. Fixed by bundling `leaflet` as a real npm
  dependency in both apps (CSS imported directly; Vite/Metro both bundle it
  same-origin) and, for mobile, replacing the iframe/`postMessage` design
  entirely with Leaflet mounted straight into a plain `div` — no iframe, no
  CSP inheritance question left to answer. Verifying this with a real
  production-mode export surfaced a second, latent bug in *both* files: the
  one marker that used Leaflet's own default icon (not a custom `divIcon`)
  depends on a relative `url(images/marker-icon.png)` inside `leaflet.css`
  that neither Metro nor Vite resolves when the CSS is bundled locally — it
  previously "worked" only because the CDN-hosted `leaflet.css` had that URL
  resolve to a real unpkg path. Both markers now use an explicit `divIcon`
  instead. Verified with a real `expo export --platform web` and
  `vite build`: the exported HTML references the CSS/JS same-origin, and
  `unpkg.com` no longer appears anywhere in `apps/`.
- **Bulk product import script.** `apps/api/prisma/import-products.ts`
  (`npm run import:products`) reads a CSV, validates every row (required
  cost price, field length/range limits mirroring `CreateMenuItemDto`,
  in-file duplicate SKU/barcode, and a cross-check against the database for a
  barcode already claimed by a *different* product) before writing anything,
  then imports every row inside one transaction — an unexpected failure mid
  -import rolls back the whole batch rather than leaving the catalogue half
  updated. Re-running the same file is safe: an existing SKU updates in
  place, a new one is created, and a category name not yet in the store is
  created once and reused. `prisma/product-import-template.csv` is the
  sample template.

### Verified

- `npm run typecheck`, `npm run lint`, `npm test` clean across all three
  workspaces (API **357 pass, 3 skipped, 0 fail**; admin 3 pass; mobile 70
  unit + 17 UI pass). `npm run build` for `@wasel/admin` (Vite) and
  `expo export --platform web` (mobile) both succeed with Leaflet bundled
  locally and no CDN references.
- **Item 1, live**: reset the local dev database
  (`prisma migrate reset --force`, explicit user consent obtained first),
  reseeded, confirmed all 5 JOVO MARKET products now carry a cost price and
  appear in the public catalogue, logged in as the seeded customer, and
  placed a real cash order (`POST /orders`, `201`, correct total) — the store
  is browsable and orderable end to end from a fresh seed.
- **Item 4, live**: registered a real (fake-content) Expo push token for the
  JOVO MARKET owner through the actual `POST /users/me/push-tokens` endpoint,
  placed an order as the customer, and confirmed in the API log that
  `PushSenderService` made a real outbound call to `exp.host` within ~450ms
  of the order committing — Expo's own server rejected the fake token
  ("is not a valid Expo push token"), proving the live round trip, that a
  bad token doesn't fail the order, and that the send is not on the request's
  critical path.
- **Item 8, live**: dry-ran and then ran the importer against the sample
  template (3 created), re-ran the same file (3 updated, 0 created — the
  SKU-upsert path), imported a row with a brand-new category name (created
  once), then ran a batch with one row missing its cost price and one row
  whose barcode collided with an existing product — confirmed the whole
  batch was rejected with both reasons reported and nothing written for
  either row, including the individually-valid one.
- **Item 3**: `docker compose --env-file ... -f docker-compose.production.yml
  config` renders cleanly (confirmed the nested `${ADMIN_TLS_CERTIFICATE_PATH
  :-${TLS_CERTIFICATE_PATH}}` default works). Built the `apps/admin/Dockerfile`
  image directly with `docker build`, ran it standalone with a throwaway
  self-signed cert pair and an `--add-host api:127.0.0.1` stand-in for the
  compose network, and confirmed: Docker's own `HEALTHCHECK` reports
  `healthy`, and `/healthz` returns `ok` over both HTTP and HTTPS from inside
  the container.
- **Item 2**: confirmed `bootstrap-admin.ts` correctly refuses against the
  live dev database, which already has a seeded `ADMIN` ("an administrator
  account already exists"). The create-a-fresh-admin happy path was not
  exercised against a live, adminless database — doing so would have meant
  deleting the seeded demo admin — but is covered by a standalone TypeScript
  compile check and by reading through `admin.service.ts`'s equivalent,
  already-tested `createAdminUser` logic that it mirrors.

### Known gaps / not done

- The push sender has no test coverage for a real (non-fake) Expo push
  token succeeding — only the failure/deactivation paths were exercised,
  since no real device was available in this environment.
- No EAS build was actually run (no EAS account session in this
  environment); `app.json`/`eas.json`/the native manifest were verified by
  reading and by a clean typecheck, not by a real `eas build`.
- The admin console's Dockerfile pins one extra native package
  (`@rollup/rollup-linux-x64-gnu@4.62.4`) to the exact version already
