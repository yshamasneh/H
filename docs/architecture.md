...
Events emitted: `order.created` (to the restaurant's room and to `admins`), `order.status.changed` and `order.fulfillment.changed` (to the authorized order room), `delivery.status.changed` (to the order room), `restaurant.pending.created` (to `admins`, on new restaurant registration), and `notification.created` (to the recipient's user room, from the shared notification helper).

**REST remains the only source of truth.** No client — admin, mobile, or otherwise — ever applies a socket payload directly to its state; every handler treats the event purely as a "something changed, go re-fetch over REST" signal (see `apps/admin/src/socket.ts` and `apps/mobile/src/core/socket.ts`). Phase 9's `useOrderRealtime` hook first emits `order.subscribe` for the visible order, then re-fetches after authorized order/delivery events and cleans up listeners when the screen changes. This means a client that missed events entirely (backgrounded, reconnecting, cold start) is always correct once it re-fetches — nothing depends on socket delivery guarantees.

## Location pricing and promotions (Phase 10)

`Restaurant.latitude/longitude` form the delivery origin and `Order.deliveryLatitude/Longitude` form the immutable destination snapshot. `calculateOrderFees()` computes Haversine distance, applies the configured minimum/included distance plus each additional started kilometer, and rejects orders beyond the maximum service radius. `POST /orders/quote` exposes the same calculation for checkout review; `POST /orders` repeats it within order creation, so the quote is informative rather than trusted state.

`Offer` supports `PRODUCT_PERCENTAGE`, `ORDER_PERCENTAGE`, `DELIVERY_PERCENTAGE`, and `FREE_DELIVERY`, optional restaurant scope for delivery campaigns, item scope for product campaigns, date bounds, minimum subtotal, maximum discount, active state, and the creating admin. Only `/admin/offers` can mutate it. The public catalog exposes active offers and decorates menu items, while `OrdersService` re-reads active eligible rows and persists winners in `Order.promotionSnapshot`.

The pricing boundary is server-only: clients send item ids, quantities, and coordinates, never delivery fees, discounts, or totals. Coordinates pass DTO and database range/pair checks. No external maps or routing service is called, and no background location stream exists.

## Supermarket catalog and inventory (Phase 11)

`Restaurant.businessType` is a discriminator rather than a second owner model. `/restaurants` and restaurant menu routes require `RESTAURANT`; `/supermarkets`, `/supermarkets/:id/catalog`, and `/supermarkets/:id/products/:productId` require `SUPERMARKET`. Both domains retain the same admin approval, owner workspace, orders, offers, deliveries, and role guards.

The grocery catalog extends `MenuItem` with `sku`, `brand`, `unitLabel`, nullable `stockQuantity`, and `isFeatured`. Catalog queries only traverse active departments and available, non-zero products. Search is performed server-side across name, description, brand, and SKU, and department counts describe currently available products.

Within the order transaction, requested quantities are aggregated by product. Tracked rows are decremented with a conditional `updateMany` requiring sufficient stock, which prevents concurrent buyers from overselling even if both received valid quotes. Cancellation or rejection increments the quantities stored in immutable `OrderItem` rows. `unitLabelSnapshot` and `allowSubstitution` travel with the existing name/price snapshots; no automatic substitution or post-order price mutation occurs.

## Grocery fulfillment and procurement (Phases 12-13)

`FulfillmentAdjustment` is a one-to-one extension of an order line. The supermarket proposes a replacement and/or packed quantity while the order is `PLACED`; the customer owns the approve/reject mutation. Pending proposals block store acceptance. REST responses are authoritative and `order.fulfillment.changed` only triggers a re-fetch through the existing authorized order socket room.

Tracked inventory is changed only inside domain transactions. `InventoryMovement` is the append-only explanation layer, while `MenuItem.stockQuantity` remains the current balance. `Supplier`, `PurchaseOrder`, and `PurchaseOrderItem` belong to one supermarket; receiving uses a conditional state transition and atomic increments, then records movement and audit rows before commit. The `InventoryModule` exposes these operations only under authenticated restaurant-owner routes and verifies `BusinessType.SUPERMARKET` plus store ownership again in the service.
